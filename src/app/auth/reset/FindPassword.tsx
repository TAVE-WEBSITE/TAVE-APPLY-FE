...
const FindPassword = () => {
    const { name, birth, email, authCode, setName, setBirth, setEmail, setAuthCode, setCurrentStep } =
    ...
    const isActive = name !== '' && birth !== '' && email !== '' && authCode !== '' && isPassed;
    ...
    return (
        <FlexBox className="pt-4 gap-8" direction="col">
            <InputContainer label="이름" isRequired={true}>
                <InputField value={name} setValue={setName} placeholder="이름을 입력해주세요"></InputField>
            </InputContainer>
            <InputContainer label="생년월일" isRequired={true}>
                <InputField value={birth} setValue={setBirth} placeholder="YYMMDD"></InputField>
            </InputContainer>
            <InputContainer label="이메일 주소" isRequired={true} description="자주 사용하는 이메일로 입력해주세요 :)">
                <FlexBox className="items-center justify-between">
                    <InputField
                        ...
                        hasButton={true}
                    />
                    <ButtonAuth text="인증요청" onClick={handleVerifyEmail} isLoading={isVerifyEmailLoading} />
                </FlexBox>
            </InputContainer>
            ...
            <div className="flex flex-col-reverse md:flex-row font-bold md:justify-end py-8 gap-1">
                <ButtonNavigate text="다음" onClick={() => setCurrentStep(2)} isActive={isActive} />
            </div>
        </FlexBox>
    );
};

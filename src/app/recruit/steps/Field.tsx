...
const Field = () => {
    ...
    const { applyApplicationQuestion, applyTempApplication, postTempApplication, postResume } = useRecruit();
    ...
    const programmingTitle = `${username}님의 프로그래밍 또는 디자인 실력은 어느 정도 인가요?`;
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            const languageData = tempData.page2.languageLevels;
            setLanguageLevels(languageData);

            const firstZeroIndex = languageData.findIndex((l: LanguageLevel) => l.level === 0);
            setVisibleCount(firstZeroIndex === -1 ? languageData.length : firstZeroIndex + 1);
        };
